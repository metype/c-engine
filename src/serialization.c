...
serialization_func(J_get_serialization_func_from_str(const char* id)) {
    if(!registered_serialization_types) return nullptr;
    return map_get(registered_serialization_types, id, serialization_func());
}

deserialization_func(J_get_deserialization_func_from_str(const char* id)) {
...
void* J_file_to_obj(FILE* file) {
    ...
    return J_block_to_obj(file_data);
}
...
void* J_block_to_obj(string* str) {
    ...
    if(block_count != 0) {
        L_printf(LOG_LEVEL_ERROR, "Failed to parse block! Block not closed! Raw Block Data: \"%s\"", str->c_str);
        return nullptr;
    }
    ...
    while(true) {
        if(buf_idx >= 64) {
            L_print(LOG_LEVEL_ERROR, "Failed to parse! Type specifier exceeded length limit!");
            return nullptr;
        }
    ...
    deserialization_func(deserializer) = J_get_deserialization_func_from_str(type_buf);

    if(!deserializer) {
        L_print(LOG_LEVEL_ERROR, "Failed to parse! Type specifier does not have properly registered funcs!");
        return nullptr;
    }
    ...
    return J_deserialize(block_str_data, deserializer);
}
...
void J_register_base_types() {
    J_register_serialization_func("int", &int_serialize);
    J_register_deserialization_func("int", &int_deserialize);

    J_register_serialization_func("bool", &bool_serialize);
    J_register_deserialization_func("bool", &bool_deserialize);

    J_register_serialization_func("float", &float_serialize);
    J_register_deserialization_func("float", &float_deserialize);

    J_register_serialization_func("char*", &str_serialize);
    J_register_deserialization_func("char*", &str_deserialize);
}